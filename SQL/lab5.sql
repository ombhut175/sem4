...
CREATE TRIGGER TR_PERSONINFO_CALCULATE_AGE
ON PersonInfo
AFTER INSERT
AS
BEGIN
...
CREATE TRIGGER TR_PERSONINFO_LIMIT_SALARY_DECREASE
ON PersonInfo
AFTER UPDATE
AS
BEGIN
    ...
    SELECT @PersonID = PersonID, @OldSalary = Salary
    FROM deleted;
    ...
    BEGIN
        ...
        PRINT 'Salary decrease is restricted to a maximum of 10%.';
    END
END;
...
-- during an INSERT.
CREATE TRIGGER TR_PERSONINFO_SET_JOININGDATE
ON PersonInfo
AFTER INSERT
AS
BEGIN
