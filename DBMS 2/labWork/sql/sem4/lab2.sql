...
AS
BEGIN
    SELECT P.*, D1.DepartmentName, D2.DesignationName
    FROM Person P
...
BEGIN
    SELECT TOP 3
        P.*, D1.DepartmentName, D2.DesignationName
    FROM Person P
        join Department D1 ON P.DepartmentID = D1.DepartmentID
        JOIN Designation D2 ON P.DesignationID = D2.DesignationID
END
...
AS
BEGIN
    SELECT P.FirstName, P.Salary, P.JoiningDate, D.DepartmentName, D2.DesignationName
    FROM Person P
        JOIN Department D ON P.DepartmentID=D.DepartmentID
        JOIN Designation D2 ON P.DesignationID=D2.DesignationID
...
AS
BEGIN
    SELECT D.DesignationName, AVG(P.Salary) AVG_SALARY, SUM(P.Salary) AS SUM
    FROM PERSON P
...
BEGIN
    ...
    WHERE D.DesignationName=@DesignationName
